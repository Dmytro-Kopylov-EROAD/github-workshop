...
The header includes a keyboard-accessible **High contrast** toggle that strengthens text, surface, border, link, and focus contrast across every page. The preference is stored in the browser and applied before page content renders, so it persists across navigation and reloads without a theme flash. The default dark theme remains available when the preference is off or JavaScript is unavailable.
